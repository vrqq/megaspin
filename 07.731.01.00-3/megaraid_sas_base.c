...
void megasas_blk_limits_max_hw_sectors(struct queue_limits *limits,
		  ...
		  const volatile void __iomem *addr)
{
	 ...
	 * upto thirty times until a non-zero value is read.
	 */
...
scsi_timeout_action megasas_reset_timer(struct scsi_cmnd *scmd)
#else
...
megasas_dump_fusion_io(struct scsi_cmnd *scmd)
{
	...
	cmd = megasas_priv(scmd)->cmd_priv;
	instance = (struct megasas_instance *)scmd->device->host->hostdata;
...
static MAP_QUEUES_RET_TYPE megasas_map_queues(struct Scsi_Host *shost)
{
	struct megasas_instance *instance;
	int qoff, offset;
...
const struct attribute_group *megaraid_host_groups[] = {
...
static inline void megasas_init_ctrl_params(struct megasas_instance *instance)
{
	...
	// megasas_dbg_lvl = 0b111; // uncommit this one to enable verbose log.
	megasas_dbg_lvl = 0;
	instance->flag = 0;
	instance->unload = 1;
...
// dirty patch: hard code pd_count in this version
//		if controller do not have the number of physical disk, it will return 
// 		error then we can break this function.
//
static int
