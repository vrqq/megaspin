...
void build_mpt_mfi_pass_thru(struct megasas_instance *instance,
			    ...
			    struct megasas_cmd_fusion *cmd)
{
	...
	if (r1_cmd->cmd_completed) {
		...
		map_cmd_status(fusion, scmd, status, ex_status,
			       le32_to_cpu(data_length), sense);
		if (instance->ldio_threshold &&
		    megasas_cmd_type(scmd) == READ_WRITE_LDIO)
			atomic_dec(&instance->ldio_outstanding);
		megasas_priv(scmd)->cmd_priv = NULL;
		megasas_return_cmd_fusion(instance, cmd);
		scsi_dma_unmap(scmd);
		megasas_sdev_busy_dec(instance, scmd);
		SCMD_DONE(scmd);
	}
}
	...
	while (d_val.u.low != cpu_to_le32(UINT_MAX) &&
		...
		case MEGASAS_MPI2_FUNCTION_LD_IO_REQUEST: /* LD-IO Path */
			...
			if (cmd_fusion->r1_alt_dev_handle == MR_DEVHANDLE_INVALID) {
				map_cmd_status(fusion, scmd, status,
